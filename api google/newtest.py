...
def generate_generic_reply(prompt):
    try:
...
def find_contact(state):
    ...
    try:
        ...
        if result.get("results"):
            ...
            st.session_state["chat_history"].append({"role": "assistant", "content": f"✅ Contact found: {contact.get('firstname', '')} {contact.get('lastname', '')} ({contact.get('email', '')})"})
        else:
            state["error"] = "Contact not found in HubSpot."
            st.session_state["awaiting_email_input"] = True
            st.session_state["chat_history"].append({"role": "assistant", "content": "❌ Contact not found in HubSpot.\n\nPlease enter the contact's email in chat."})
    except Exception as e:
...
"""
    ...
    st.session_state["chat_history"].append({"role": "assistant", "content": f"✍️ Generated email:\n\n```text\n{email_content.strip()}\n```"})
    return state
    ...
    if st.session_state.get("awaiting_email_input") and "@" in prompt:
        ...
        if email_info:
            ...
            with st.spinner("Thinking..."):
                reply = generate_generic_reply(prompt)
